 ...
 * 복사 생성자 
 * 
 * 복사 생성자란 객체가 복사될 때 자동으로 호출되는 함수를 의미한다. 먼저 monster_b 클래스의 객체를
 * 2개 생성하는 코드를 살펴보자. monster_b는 수수께끼를 내는 몬스터이다. 
 */
...
/**
 * 실행 결과
 * [30, 30] 첫 번째 몬스터(수수꼐끼 몬스터 - 100)가 내는 수수께끼: 아침에 네 발, 점심에는 두 발, 저녁에는 두 발인 것은?
 * [45, 50] 두 번째 몬스터(일반 - 10)가 내는 수수께끼: 문이 뒤집어 지면 무엇이 될까?
 */

/**
 * 이제 second_mon 몬스터의 타입과 난이도를 first_mon 몬스터와 똑같이 설정하고 싶다. first_mon 몬스터처럼 모든 설정 함수를
 * 호출할 수도 있지만, monster_b second_mon = first_mon 처럼 객체를 생성할 때 똑같은 클래스로 생성한 기존 객체를 대입하는
 * 방법도 있다. 그러면 멤버 변수들이 그대로 복사되므로 수수께끼와 몬스터 위치만 다르게 설정해 주면 된다. 
 */