<template>
    <v-container fluid class="d-flax justify-center align-center pa-0"
...
const goToKakaoLogin = async () => {
    seesionStorage.setItem("loginType", "KAKAO");
...
const onSubmit = async () => {
    ...
    try {
        ...
        const roleType = await requestKakaoLoginToDjango(email.value);
        ...
        if (response) {
            ...
            if (roleType.data.roleType === "ADMIN") {
                // Admin 처리
                sessionStorage.removeItem('normalType');
                sessionStorage.setItem('adminToken', true);
                account.REQEST_IS_ADMIN_TO_DJANGO(true);
                goToHome();
            } else {
                // Normal 처리
                sessionStorage.setItem('normalType', true);
...
/* 눈 아이콘 색상 설정 */
:deep(.mdi-eye::before),
:deep(.mde-eye-off::before) {
    color: rgba(255, 255, 255, 0.8) !important;
}

/* 오류 메시지 스타일링 */
:deep(.v-messates__message) {
